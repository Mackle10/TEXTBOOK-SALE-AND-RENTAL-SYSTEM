...
| --- | --- |
| User authentication | Student email registration & login (`users` table) |
...
3. **Configure** — edit `config/config.php` if needed:
   - `UNIVERSITY_EMAIL_DOMAIN` — legacy constant retained for compatibility; registration now accepts any valid student email address. You can remove or ignore this constant if you don't use domain restrictions.
...
| Email | Password | Role |
|---|---|---|
| mark@student.example | password | Seller |
| jane@student.example | password | Buyer |
...
├── register.php      Student email signup
